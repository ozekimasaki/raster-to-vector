...
- `--despeckle N`：8近傍の同色が N 個未満の画素を近傍の最頻ラベルへ付け替える。ただし同色近傍に十分支持された画素が1つでもあれば温存するため、N=2 では1px線（対角線を含む）が一切削れない。推奨は `2`。N≧3 は対角1px線の内部画素まで弱体化させ得る。既定 0（無効。ピクセルアート等、1px が意味を持つ入力ではそのままにする）。
