...
依頼: 「この塗り絵風イラストをSVGに。隣り合う色の隙間をなくして」
analyze→目視→`mosaic-draft`（polygon または pixel）→shared-boundary.json で辺の共有を確認→必要なら意味でグループを編集→check。
ドラフトを最終合格にしない。幾何が接していても描画の AA ヘアラインは残り得る。
