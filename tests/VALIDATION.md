...
日本語のSKILL.md、用途別参照9文書、３理論文書の無変更コピー、補助CLI9コマンド、
既存CFV-Xの任意ドラフト経路、格子 mosaic-draft、原実験コード、固定サンプル、単体・受け入れ試験を収録。
既存資材は変更せず、source-manifestで同梱コピーのハッシュを照合した。
SVGサブセット検査、実描画、背景別差分、alpha欠損・過剰、品質の未検証状態を実装。

## 確認した結果

- 挙動テスト34件合格。ICC・EXIF・フレーム選択・透明色・alpha保存・異常入力・参照拒否・timeout・環境不足に加え、mosaic の格子往復・T字・pinch・入れ子 ring を含む。
...
画素格子上の共有境界（mosaic-draft）は追加設計であり、汎用DCELの代替ではない。
